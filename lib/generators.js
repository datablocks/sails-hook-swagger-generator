...
const formatter = require("./type-formatter");
const fs = require('fs');
...
function _generateParameters(config, context) {
    ...
    params.TokenHeaderParam = {
        ...
        required: false,
        type: 'string',
        description: 'In case we want to send header information along our request'
    };
...
function _generateRoutes(controllers, config, tags) {
    ...
    controllers = _.cloneDeep(controllers);//copy the value out without letiable referencing
    //building all our routes from the custom since the custom over writes the default route
    let routes = parsers.routes(config.routes); // make the key here the controller name if available?
    // routes['user'] = routes['']
    // delete routes['']
    ...
    _.forEach(controllers, function (controller, identity) {
        ...
        let blueprints = _.defaults(config.blueprints, {
            rest: true
        });

        let prefix = (blueprints.prefix && blueprints.prefix !== '' ? blueprints.prefix : '') + (blueprints.restPrefix && blueprints.restPrefix !== '' ? blueprints.restPrefix : '');
        if (prefix !== '') prefix += '/';
        ...
        let exclude = ['identity', 'globalId', 'sails'];
        let excludeShortcuts = ['BLUEPRINT: populate', 'ACTION: security/grant-csrf-token', 'BLUEPRINT: findone', 'BLUEPRINT: add', 'BLUEPRINT: remove', 'BLUEPRINT: replace'];

        _.forEach(controller, function (value, action) {
            if (exclude.indexOf(action) > -1 || !sails.config.blueprints.shortcuts && excludeShortcuts.indexOf(value._middlewareType) > -1)
                return;
            ...
            if (_.findIndex(routes[identity], { 'action': action }) < 0) {
                let policies = sails.config.policies[[identity,action].join('/')] || sails.config.policies[[identity,'*'].join('/')]
                routes[identity].push({
                    ...
                    summary: value.summary + (policies ? ' __,.-\'~\'-.,__,.-\'~\'-.,__ Policy:' + policies :''),
...
function _generatePaths(generated_routes, tags, customBlueprintMaps) {
    // todo
    // remove createdAt, updatedAt,id, from create generation routes in swagger somehow
    // remove Archive model
    //  tags.push('')

    let controllersFiles = fs.readdirSync(path.join(sails.config.appPath, 'api', 'controllers'));   // Read Controllers directory to prepare Controller Tags
    controllersFiles.forEach((file)=>{
        if(file.startsWith('.'))  // dont read .gitkeep files
            return
        if(file.indexOf('Controller')>-1)
            file = file.substring(0,file.indexOf('Controller'))

        if(!_.find(tags, { identity: file.toLowerCase() })){
            tags.push({
                name: file,
    ...
    _.remove(tags, { name: 'Archive' })

    tags.push({ name: 'Auth', identity: 'auth' })
    ...
    _.forEach(generated_routes, function (routes, identity) {
        _.forEach(routes, function (route) {
            ...
            let tag = _.find(tags, { identity: identity });
            ...
            if (!tag) {
                ...
                tag = { name: 'View Routes', identity: 'routes' }
            }
            ...
            let link = _standardiseLink(route.path + "/" + _processRouteKeys(route.keys));
...
function _processRouteKeys(keys) {
    ...
    let map = _.map(keys, function (key) {
